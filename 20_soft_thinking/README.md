...
There's a fundamental tension: the model was **trained** on discrete token embeddings (points on the embedding manifold), but concept tokens are weighted averages that may lie **between** those points — regions the model has never seen. This is the train-test mismatch that Lab 21 addresses.
