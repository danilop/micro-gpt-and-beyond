...
B starts as all zeros, so the adapter's initial output is `B @ A @ x = 0`. The model begins fine-tuning from exactly its pre-trained behavior — no random perturbation, stable training from step one. The adaptation builds up gradually as B learns non-zero values.
...
3. **Inject LoRA** adapters into all Linear layers in attention and MLP — print total vs trainable parameter counts
4. **Fine-tune** with LoRA for 500 steps on the filtered subset, training only the adapter parameters
5. **Compare** generation from the base model vs the LoRA-adapted model — the distribution shift is visible
...
Because LoRA adapters are small and the base model stays frozen, you can train separate adapters for different tasks and swap them at serving time. One base model, many behaviors — each adapter is just a pair of small matrices per layer.
