...
LABS = [
    ...
    ("05_jax", "main.py",
     "JAX",
     "Purely functional style. No classes, no hidden state, no mutation. Every function takes its inputs and returns its outputs."),
    ("06_jax_batched", "main.py",
     "JAX Batched",
     "Use jax.vmap to automatically vectorize single-example code across a batch. No rewriting needed."),
    ("07_mlx", "main.py",
     "MLX (Apple Silicon)",
     "Running on Apple Silicon GPU via MLX. A NumPy-like API with automatic differentiation and unified memory."),
    ("08_mlx_batched", "main.py",
     "MLX Batched",
     "Mini-batch training on Apple Silicon, following the same padding and masking approach as PyTorch batched."),
    ("09_bpe_tokenizer", "main.py",
     "BPE Tokenizer",
     "Byte-Pair Encoding from scratch in pure Python, with zero dependencies."),
    ("10_rope", "main.py",
     "Rotary Position Embeddings (RoPE)",
     "Position encoded by rotating Q and K vectors in complex space. Relative position without learned parameters."),
    ("11_gqa", "main.py",
     "Grouped-Query Attention (GQA/MQA)",
     "From MHA to MQA to GQA: sharing KV heads reduces memory during inference while preserving quality."),
    ("12_kv_cache", "main.py",
     "KV Cache",
     "The single most important inference optimization. Cache K and V tensors so each new token only processes one position."),
    ("13_sampling", "main.py",
     "Sampling Strategies",
    ...
    ("18_tiled_attention", "main.py",
     ...
     "Three attention implementations producing identical outputs. The difference is memory trips. This is why FlashAttention matters."),
    ...
    ("21_soft_training", "main.py",
     "Soft Training",
     "Use concept tokens during training too. A curriculum that closes the train-test gap from inference-only soft thinking."),
