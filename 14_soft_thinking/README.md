...
There's a fundamental tension: the model was **trained** on discrete token embeddings (points on the embedding manifold), but concept tokens are weighted averages that may lie **between** those points — regions the model has never seen. This is the train-test mismatch that Lab 15 addresses.
...
- Why this is training-free — only the decoding loop changes, not the model
...
- **Cold Stop mechanism**: Monitor entropy during soft decoding; when it drops below a threshold (model is very confident), switch back to hard decoding. Prevents OOD drift.
...
Large language models spend enormous compute producing rich output distributions at every step, only to throw almost all of it away by picking a single token. Soft thinking is the insight that this information doesn't have to be wasted — the full distribution can flow forward as a continuous signal. At scale, this improves reasoning quality (+2.5 pass@1) while using fewer tokens (-22%). This lab shows the core mechanism at microGPT scale: one line of code (`softmax @ embedding_table`) is the entire difference.
