...
During inference, the dominant memory cost is not the model weights or the compute — it is the **KV cache**. Every generated token requires storing key and value vectors for all previous positions, across all layers and all heads. For long sequences and large batch sizes this cache dwarfs everything else.
...
When `n_kv_head == n_head`, `repeats == 1` and nothing is repeated — you get standard MHA. When `n_kv_head == 1`, every KV vector is broadcast to all query heads — MQA. Anything in between is GQA.
