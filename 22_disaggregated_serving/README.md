...
LLM inference has two phases with fundamentally different hardware profiles. Prefill processes the entire prompt in parallel — it's compute-bound, saturating GPU ALUs. Decode generates tokens one at a time — it's memory-bandwidth-bound, with the GPU mostly idle waiting to read weights from memory. When both phases share the same GPU, a long prefill (e.g., a 32K-token document) blocks all decode requests, causing latency spikes for users waiting for their next token. Disaggregation fixes this by running each phase on separate, optimized hardware.
...
This means you can use different GPU types: compute-dense GPUs for prefill, bandwidth-optimized GPUs for decode. Or even different numbers — one prefill GPU can feed several decode GPUs.
...
- NVLink bandwidth: 900 GB/s → ~1.1 ms transfer time
...
- **Continuous batching**: Iteration-level scheduling where finished requests are immediately replaced. Orthogonal to disaggregation — production systems combine both.
